/**
 * @author Paúl Santapau 
 * 
 * The code below uses the iText library with Aiken Sam and  Martin Brunecky
 * patch for embedding timestamps applied, for more information you can 
 * see 
 * 
 *  http://www.nabble.com/PDF-Digital-signature-with-timestamp---using-Time-Stamp-Authority---example-td11798693.html
 * 
 * The code that generates the signed pdf with timestamp has been tunned from 
 * Martin's one.
 * 
 */
...
public class PDFSignatureFactory extends AbstractSignatureFactory implements ISignFormatProvider {
	...
	protected byte[] genPKCS7Signature(InputStream data, TSAClient tsc, PrivateKey pk, Provider pv, Certificate[] chain )
	throws  Exception {
	...
	private void signPdfTsp(PdfSignatureAppearance sap) throws Exception{
		...
		tsc = new TSAClientBouncyCastle(prop.getProperty("PDFSIG_TSA_URL"),
				prop.getProperty("PDFSIG_TSA_ACCOUNT"), prop.getProperty("PDFSIG_TSA_PWD"));


		byte[] estSignature = genPKCS7Signature(new ByteArrayInputStream("fake".getBytes()), null,pk,pv,chain);
	...
	public byte[] formatSignature(byte[] datos, X509Certificate sCer, PrivateKey pk, Provider pv) throws KeyStoreException, Exception
	{
		try{
			...
			for (int i=1; i<= n; i++)
			{	
				CACert= cf.generateCertificate(cl.getResourceAsStream(prop.getProperty("PDFSIG_CA_CERT"+i)));
				try{
