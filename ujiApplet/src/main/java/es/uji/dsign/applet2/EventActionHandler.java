...
public class EventActionHandler {
	 ...
	 * 
	 * @param mw The mainWindow of the program.
	 */
	...
	public java.awt.event.ActionListener getAboutActionListener()
	{
		ActionListener res= new java.awt.event.ActionListener() {
			...
			{
				try {
					JOptionPane.showMessageDialog(mw.getMainFrame(), LabelManager.get("ABOUT_DESCRIPTION"), "", JOptionPane.INFORMATION_MESSAGE);
	...
	public java.awt.event.ActionListener getHelpActionListener()
	{
		ActionListener res= new java.awt.event.ActionListener() {
			...
			{
				try {
					JOptionPane.showMessageDialog(mw.getMainFrame(), LabelManager.get("HELP_DESCRIPTION"), "", JOptionPane.INFORMATION_MESSAGE);
				} catch (HeadlessException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				} catch (Exception e1) {
	...
	/**
	 * Creates the listener that handles all Certificate Tree Selector complexity like 
	 * show the pin filed when a certificate is selected, show the certificate info, etc. 
	 * 
	 * @return tsl 
	 */
	...
	/**	
	 * 
	 * Implements the pkcs#12 menu itme action listener that fires an open file dialog when 
	 * pressed.
	 * 
			...
			public void actionPerformed(java.awt.event.ActionEvent e)
			{
				try{
					...
					if(returnVal == JFileChooser.APPROVE_OPTION) {
						...
						if ( ! pkFile.exists() ){
							JOptionPane.showMessageDialog(mw.getMainFrame(), LabelManager.get("ERROR_FILE_NOT_EXISTS"), "", JOptionPane.ERROR_MESSAGE); 
						}
						else{
							try
							{
							...
							{
								JOptionPane.showMessageDialog(mw.getMainFrame(), LabelManager.get("ERROR_OPEN_PKCS12"), "", JOptionPane.ERROR_MESSAGE); 
								exc.printStackTrace();
							}
						}
					}
	...
	/**	
	 * 
	 * Implements the pkcs#11 menu itme action listener that fires an open file dialog when 
	 * pressed.
	 * 
	 * @return ActionListener 
	 */
			...
			public void actionPerformed(java.awt.event.ActionEvent e)
			{
				try{
					...
					if(returnVal == JFileChooser.APPROVE_OPTION) {
						...
						File pkFile= chooser.getSelectedFile().getAbsoluteFile();
						if ( ! pkFile.exists() ){
							JOptionPane.showMessageDialog(mw.getMainFrame(), "No se encontró fichero", "", JOptionPane.ERROR_MESSAGE); 
						}
						else{
							try
							{
								PKCS11KeyStore pkStore= new PKCS11KeyStore(pkFile.getAbsolutePath());
							...
							{
								JOptionPane.showMessageDialog(mw.getMainFrame(), LabelManager.get("ERROR_OPEN_PKCS11"), "", JOptionPane.ERROR_MESSAGE); 
								exc.printStackTrace();
							}
						}
					}
