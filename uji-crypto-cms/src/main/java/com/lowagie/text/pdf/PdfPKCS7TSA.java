...
public class PdfPKCS7TSA
{
    ...
    public PdfPKCS7TSA(byte[] contentsKey, byte[] certsKey, String provider)
            ...
            NoSuchProviderException, NoSuchAlgorithmException, IOException, StreamParsingException,
     ...
     * @throws org.bouncycastle.x509.util.StreamParsingException
     */
    public PdfPKCS7TSA(byte[] contentsKey, String provider) throws SecurityException, CRLException,
            ...
            org.bouncycastle.x509.util.StreamParsingException
    {
        ...
        ASN1Sequence content = (ASN1Sequence) ((DERTaggedObject) signedData.getObjectAt(1))
                .getObject();
        ...
        digestAlgorithm = ((DERObjectIdentifier) ((ASN1Sequence) signerInfo.getObjectAt(2))
                .getObjectAt(0)).getId();
        next = 3;
    ...
    public PdfPKCS7TSA(PrivateKey privKey, Certificate[] certChain, CRL[] crlList,
            String hashAlgorithm, String provider, boolean hasRSAdata) throws SecurityException,
    ...
    /**
     * Update the digest with the specified bytes. This method is used both for signing and
     * verifying
     * 
     * @param buf
     *            the data buffer
    ...
    public String getDigestAlgorithm()
    {
        ...
        if (digestEncryptionAlgorithm.equals(ID_RSA) || digestEncryptionAlgorithm.equals(ID_MD5RSA)
                || digestEncryptionAlgorithm.equals(ID_MD2RSA)
                || digestEncryptionAlgorithm.equals(ID_SHA1RSA))
        {
            dea = "RSA";
        }
    ...
    /**
     * Loads the default root certificates at &lt;java.home&gt;/lib/security/cacerts with the
     * default provider.
     * 
     * @return a <CODE>KeyStore</CODE>
     * @throws IOException 
     */
    public static KeyStore loadCacertsKeyStore() throws IOException
    {
     ...
     * @throws IOException 
     */
    public static KeyStore loadCacertsKeyStore(String provider) throws IOException
    {
     ...
     *         <CODE>Object[]{cert,error}</CODE> where <CODE>cert</CODE> is the failed certificate
     *         and <CODE>error</CODE> is the error message
     */
    public static Object[] verifyCertificates(Certificate certs[], KeyStore keystore,
     ...
     * @throws IOException 
     */
    private static DERObject getIssuer(byte[] enc) throws IOException
    {
        ASN1InputStream in = new ASN1InputStream(new ByteArrayInputStream(enc));
        ASN1Sequence seq = (ASN1Sequence) in.readObject();
        return (DERObject) seq.getObjectAt(seq.getObjectAt(0) instanceof DERTaggedObject ? 3
                    : 2);
    }
     ...
     * @throws IOException 
     */
    private static DERObject getSubject(byte[] enc) throws IOException
    {
        ASN1InputStream in = new ASN1InputStream(new ByteArrayInputStream(enc));
        ASN1Sequence seq = (ASN1Sequence) in.readObject();
        return (DERObject) seq.getObjectAt(seq.getObjectAt(0) instanceof DERTaggedObject ? 5
                : 4);
    }
     ...
     */
    public static X509Name getIssuerFields(X509Certificate cert) throws CertificateEncodingException, IOException
    {
     ...
     * @throws CertificateEncodingException 
     */
    public static X509Name getSubjectFields(X509Certificate cert) throws CertificateEncodingException, IOException
    {
     ...
     * @throws SignatureException 
     */
    public byte[] getEncodedPKCS1() throws IOException, SignatureException
    {
     ...
     *            <CODE>digest</CODE> is also <CODE>null</CODE>. If the <CODE>digest</CODE> is not
     ...
     * @throws Exception 
     */
    public byte[] getEncodedPKCS7() throws Exception
    {
        return getEncodedPKCS7(null, null, null);
    }

    /**
     * Gets the bytes for the PKCS7SignedData object. Optionally the authenticatedAttributes in the
     * signerInfo can also be set. If either of the parameters is <CODE>null</CODE>, none will be
     * used.
     * 
     * @param secondDigest
     ...
     * @throws Exception 
     */
    public byte[] getEncodedPKCS7(byte secondDigest[], Calendar signingTime, TSAClient tsaClient) throws Exception
    {
        if (externalDigest != null)
        {
            digest = externalDigest;
        ...
        if (secondDigest != null && signingTime != null)
        {
            ...
            if (!crls.isEmpty())
            {
                ...
                v.add(new DERSet(new DERSequence(new DERTaggedObject(true, 0, new DERSequence(
                        v2)))));
                attribute.add(new DERSequence(v));
            }
        ...
        if (!crls.isEmpty())
        {
            ...
            for (Iterator i = crls.iterator(); i.hasNext();)
            {
                ASN1InputStream t = new ASN1InputStream(new ByteArrayInputStream((((X509CRL) i
                        .next()).getEncoded())));
                v.add(t.readObject());
            }
    ...
    /*
     * Added by Aiken Sam, 2006-11-15, modifed by Martin Brunecky 07/12/2007 to start with the
     * timeStampToken (signedData 1.2.840.113549.1.7.2). Token is the TSA response without response
    ...
    /**
     * When using authenticatedAttributes the authentication process is different. The document
     * digest is generated and put inside the attribute. The signing is done over the DER encoded
     ...
     * @throws CRLException 
     */
    public byte[] getAuthenticatedAttributeBytes(byte secondDigest[], Calendar signingTime) throws IOException, CRLException
    {
        ASN1EncodableVector attribute = new ASN1EncodableVector();
        ASN1EncodableVector v = new ASN1EncodableVector();
        v.add(new DERObjectIdentifier(ID_CONTENT_TYPE));
        ...
        if (!crls.isEmpty())
        {
            ...
            v
                    .add(new DERSet(new DERSequence(new DERTaggedObject(true, 0,
                            new DERSequence(v2)))));
            attribute.add(new DERSequence(v));
        }
        ByteArrayOutputStream bOut = new ByteArrayOutputStream();
     ...
     * We need this class as some of the lightweight Java environment don't support classes like
     * StringTokenizer.
     */
