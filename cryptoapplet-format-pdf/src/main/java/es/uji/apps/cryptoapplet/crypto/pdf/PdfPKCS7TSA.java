...
/**
 * 10-11-2009
 * Modified by Paul -> stripped out everything that has not to do with timestamping and pre calculated ocsp. 
 * 
 * */
...
import es.uji.apps.cryptoapplet.crypto.timestamp.TimeStampFactory;
...
public class PdfPKCS7TSA
{
    ...
    public PdfPKCS7TSA(byte[] contentsKey, Provider provider)
    {
        try
        {
            ...
            BigInteger serialNumber = ((DERInteger) issuerAndSerialNumber.getObjectAt(1))
                    .getValue();
            ...
            if (signerInfo.getObjectAt(next) instanceof ASN1TaggedObject)
            {
                ...
                for (int k = 0; k < sseq.size(); ++k)
                {
                    ...
                    if (((DERObjectIdentifier) seq2.getObjectAt(0)).getId().equals(
                            ID_MESSAGE_DIGEST))
                    {
                        ASN1Set set = (ASN1Set) seq2.getObjectAt(1);
                        digestAttr = ((DEROctetString) set.getObjectAt(0)).getOctets();
                    }
                    else if (((DERObjectIdentifier) seq2.getObjectAt(0)).getId().equals(
                            ID_ADBE_REVOCATION))
                    {
    ...
    /**
     * Get the X.509 sign certificate chain associated with this PKCS#7 object. Only the
     * certificates used for the main signature will be returned, with the signing certificate
     * first.
     * 
     * @return the X.509 certificates associated with this PKCS#7 object
     * @since 2.1.6
    ...
    /**
     * Gets the bytes for the PKCS7SignedData object. Optionally the authenticatedAttributes in the
     * signerInfo can also be set, OR a time-stamp-authority client may be provided.
     * 
     * @param secondDigest
     *            the digest in the authenticatedAttributes
     * @param signingTime
     ...
     * @since 2.1.6
     */
    public byte[] getEncodedPKCS7(byte secondDigest[], Calendar signingTime, String tsaUrl,
            byte[] ocsp)
    {
        try
        {
            ...
            {
                signerinfo.add(new DERTaggedObject(false, 0, getAuthenticatedAttributeSet(
                        secondDigest, signingTime, ocsp)));
            }
            ...
            if (tsaUrl != null)
            {
                ...
                TimeStampResponse response = TimeStampFactory.getTimeStampResponse(tsaUrl, tsImprint,
                        false);
                byte[] tsToken = response.getEncoded();

                // Strip the status code out of the response, the adobe validator requieres it.
                // TODO: Research about this.
                byte[] status = { 0x30, (byte) 0x82, 0x03, (byte) 0xA7, 0x30, 0x03, 0x02, 0x01,
                        0x00 };
                byte[] modTsToken = new byte[tsToken.length - status.length];
                System.arraycopy(tsToken, status.length, modTsToken, 0, tsToken.length
                        - status.length);
            ...
            byte[] ocsp)
    {
        try
        {
            ...
            if (ocsp != null)
            {
                ...
                v.add(new DERSet(
                        new DERSequence(new DERTaggedObject(true, 1, new DERSequence(vo1)))));
                attribute.add(new DERSequence(v));
            }
