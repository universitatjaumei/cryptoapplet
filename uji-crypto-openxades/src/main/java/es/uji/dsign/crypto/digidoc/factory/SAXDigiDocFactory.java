...
import es.uji.dsign.crypto.digidoc.factory.SAXDigiDocFactory;
	...
	public SignedDoc readSignedDoc(String fileName) throws DigiDocException {
		...
		if (m_doc == null)
			throw new DigiDocException(
				...
				"This document is not in digidoc format",
				null);
		return m_doc;
	}
					...
					if(cval != null) {
						...
						try {
							...
							if(ConvertUtils.isKnownTSACert(cn)) {
								...
								//System.out.println("CertId: " + cid.getId() + " type: " + cid.getType() + " nr: " + cid.getSerial());
							}
		...
		if (qName.equals("DigestMethod")) {
			...
			try {
				if (m_tags.search("Reference") != -1) {
					...
					m_doc.getLastSignature().getSignedProperties().setCertDigestAlgorithm(Algorithm);
			...
			if (m_nCollectMode == 0) {
				...
				if(df.getContentType().equals(DataFile.CONTENT_EMBEDDED)) {
				  try {					
					...
					df.setBody(ConvertUtils.str2data(m_sbCollectChars.toString(), 	df.getInitialCodepage()));
					...
					int idx1 = 0;
					while(Character.isWhitespace(str1.charAt(idx1))) idx1++;
					//idx1 = str1.indexOf('<');
					String str2 = null;
					if(idx1 > 0) {
		...
		if (qName.equals("X509SerialNumber")
			...
			try {
				...
				if ( m_tags.search("SignedProperties") !=-1 && m_tags.search("IssuerSerial") != -1 )
					sig.getSignedProperties().setCertSerial(ConvertUtils.string2bigint(m_sbCollectItem.toString()));
