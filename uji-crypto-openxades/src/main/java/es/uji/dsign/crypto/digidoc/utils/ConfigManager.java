...
public class ConfigManager
{
    ...
    /**
     * Init method for reading the config data from a properties file. Note that this method doesn't
     * reset the configuration table held in memory. Thus you can use it multpile times and add
     * constantly new configuration entries. Use the reset() method to reset the configuration
     * table.
     * 
     * @param cfgFileName
     *            config file anme or URL
     * @return success flag
     */
    public static boolean init(String cfgFileName)
    {
        ...
        {
            System.err.println("Cannot read config file: " + cfgFileName + " Reason: "
                    + ex.toString());
        }
        // initialize
        return bOk;
    ...
    /**
     * Init method for settings the config data from a any user defined source
     * 
     * @param hProps
     *            config data
     */
    ...
    public SignatureFactory getSignatureFactory() throws DigiDocException
    {
        ...
        try
        {
            sigFac = (SignatureFactory) Class.forName(getProperty("DIGIDOC_SIGN_IMPL"))
                    .newInstance();
            sigFac.init();
        }
    ...
    public NotaryFactory getNotaryFactory() throws DigiDocException
    {
        try
        {
            if (m_notFac == null)
            {
                m_notFac = (NotaryFactory) Class.forName(getProperty("DIGIDOC_NOTARY_IMPL"))
                        .newInstance();
                m_notFac.init();
            }
        }
    ...
    public TimestampFactory getTimestampFactory() throws DigiDocException
    {
        try
        {
            if (m_tsFac == null)
            {
                m_tsFac = (TimestampFactory) Class.forName(getProperty("DIGIDOC_TIMESTAMP_IMPL"))
                        .newInstance();
                m_tsFac.init();
            }
        }
    ...
    public DigiDocFactory getDigiDocFactory() throws DigiDocException
    {
        ...
        try
        {
            digFac = (DigiDocFactory) Class.forName(getProperty("DIGIDOC_FACTORY_IMPL"))
                    .newInstance();
            digFac.init();
        }
        catch (DigiDocException ex)
        ...
        try
        {
            if (m_canFac == null)
            {
                m_canFac = (CanonicalizationFactory) Class.forName(
                        getProperty("CANONICALIZATION_FACTORY_IMPL")).newInstance();
                m_canFac.init();
            }
        }
        catch (DigiDocException ex)
    ...
    public EncryptedDataParser getEncryptedDataParser() throws DigiDocException
    {
        ...
        {
            if (m_dencFac == null)
                m_dencFac = (EncryptedDataParser) Class.forName(
                        getProperty("ENCRYPTED_DATA_PARSER_IMPL")).newInstance();
            m_dencFac.init();
        }
        catch (DigiDocException ex)
        {
    ...
    public EncryptedStreamParser getEncryptedStreamParser() throws DigiDocException
    {
        ...
        {
            if (m_dstrFac == null)
                m_dstrFac = (EncryptedStreamParser) Class.forName(
                        getProperty("ENCRYPTED_STREAM_PARSER_IMPL")).newInstance();
            m_dstrFac.init();
        }
        catch (DigiDocException ex)
        {
     ...
     */
    public CRLFactory getCRLFactory() throws DigiDocException
    {
        try
        {
            if (m_crlFac == null)
            {
                m_crlFac = (CRLFactory) Class.forName(getProperty("CRL_FACTORY_IMPL"))
                        .newInstance();
                m_crlFac.init();
            }
        }
