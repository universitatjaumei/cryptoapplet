...
public class EncryptedStreamSAXParser extends DefaultHandler implements EncryptedStreamParser
{
    ...
    /**
     * we use this two buffer system because xml parser returns parsed base64 data in various size
     * of chunks and we know that this is the last chunk only after we have met the </CipherValue>
     * tag but the row before that might also be shorter and thus require us to use cipher.doFinal()
     * instead of cpher.update(). So we constantly store away the newest buffer and work on the
     * previous one. This way we know that it's the last buffer when working on the last row.
     */
     ...
     * 
     * @see es.uji.security.crypto.openxades.digidoc.xmlenc.factory.EncryptedDataParser#init()
     */
    ...
    /**
     * Initializes the Recipient atribute value used for locating the right <EncryptedKey> to be
     * used for deryption
     * 
     * @param s
            ...
            throws SAXDigiDocException
    {
        ...
        if (tName.equals("KeyName") || tName.equals("CarriedKeyName")
                || tName.equals("X509Certificate") || tName.equals("EncryptionProperty"))
        ...
        if (tName.equals("CipherValue"))
        {
            ...
            { // child of <EncryptedData>
                ...
                if (m_logger.isInfoEnabled())
                    m_logger.info("Total input: " + m_totalInput + " decrypted: "
                            + m_totalDecrypted + " decompressed: " + m_totalDecompressed);
            }
            m_sbCollectChars = null; // stop collecting
        }
        // <EncryptionProperty>
    ...
    /**
     * Called with a block of base64 data that must be decoded, decrypted and possibly also
     * decompressed
     * 
     * @param data
    ...
    private void decryptBlock(String data, int nBlockType) throws SAXException
    {
        ...
        if (m_logger.isDebugEnabled())
            m_logger
                    .debug("IN " + ((data != null) ? data.length() : 0) + " input: "
                            + ((indata != null) ? indata.length() : 0) + " left: "
                            + ((m_leftB64 != null) ? m_leftB64.length() : 0) + " block-type: "
                            + nBlockType);
        try
        {
        ...
        try
        {
            ...
            if (m_leftB64 != null || indata != null)
                nUsed = Base64Util.decodeBlock(b64data.toString(), bos,
            ...
            if (m_logger.isDebugEnabled())
                m_logger.debug("Decoding: " + b64data.length() + " got: "
                        ...
                        + (nBlockType == DENC_BLOCK_LAST));
            /*
             * if(nBlockType == DENC_BLOCK_LAST) { if(encdata != null && encdata.length > 0) { //
             * check for extra (base64?) padding with 0xF bytes int nExtPad = 0, n = 0; do { n = new
             * Integer(encdata[encdata.length - 1 - nExtPad]).intValue();
             * if(m_logger.isDebugEnabled()) m_logger.debug("Data at: " + (encdata.length - 1 -
             * nExtPad) + " = " + n); if(n == 16) nExtPad++; } while(n == 16 && nExtPad <
             * encdata.length - 1); if(nExtPad > 0) { if(m_logger.isDebugEnabled())
             * m_logger.debug("Removing extra padding: " + nExtPad + " from: " + encdata.length);
             * byte [] tmp = new byte[encdata.length - nExtPad]; System.arraycopy(encdata, 0, tmp,
             * 0, encdata.length - nExtPad); encdata = tmp; } decdata = m_cipher.doFinal(encdata); }
             * //else decdata = m_cipher.doFinal(); } else
             */
            /*
             * if(m_totalDecrypted == 0) decdata = m_cipher.update(encdata, 16, encdata.length -
             * 16); else
             */
            decdata = m_cipher.update(encdata);

            if (m_logger.isDebugEnabled())
                m_logger.debug("Decrypted input: " + ((indata != null) ? indata.length() : 0)
                        + " decoded: " + ((encdata != null) ? encdata.length : 0) + " decrypted: "
            ...
            if (decdata != null && encdata != null && nBlockType == DENC_BLOCK_LAST)
            {
                ...
                if (nPadLen > 0)
                {
                    ...
                    if (m_logger.isDebugEnabled())
                        m_logger.debug("Decrypted: " + decdata.length + " encoded: "
                                + encdata.length + " check padding: " + nPadLen + " ext: "
                                + nExtPad);
                    boolean bPadOk = true;
                    if (nPadLen > 16 || nPadLen < 0)
                        bPadOk = false;
                    for (int i = decdata.length - nPadLen - nExtPad; bPadOk
                            && nPadLen < decdata.length && i < decdata.length - 1 - nExtPad; i++)
                    {
                        if (m_logger.isDebugEnabled())
            ...
            if (decdata != null)
            {
                ...
                if (m_decompressor != null)
                {
                    ...
                    if (m_logger.isDebugEnabled())
                        m_logger.debug("Decompressing: " + decdata.length + " into: "
                                + m_decbuf.length + " got: " + nDecomp);
