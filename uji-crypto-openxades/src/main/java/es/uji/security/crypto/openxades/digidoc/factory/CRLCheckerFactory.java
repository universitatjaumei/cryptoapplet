 ...
 * certificates. HTTP connection has the advatage of offering download only if a new version of CRL
 * exists.
 * 
...
public class CRLCheckerFactory implements CRLFactory
{
     ...
     * { X509Certificate cert = null; try { byte[] cdata = Base64Util.decode(b64cert);
     * ByteArrayInputStream inStream = new ByteArrayInputStream(cdata); CertificateFactory cf =
     * CertificateFactory.getInstance("X.509"); cert =
     * (X509Certificate)cf.generateCertificate(inStream); inStream.close(); } catch(Exception ex) {
    ...
    private X509CRL getCRL() throws DigiDocException
    {
        ...
        else
        {
            ...
            try
            {
                ...
                HttpURLConnection conn = (HttpURLConnection) new URL(m_crlUrl).openConnection();
