...
/**
 * Base64 utility methods. Based on an implementation by Apache, but changed to suit the needs of
 * this project.
 */
public class Base64Util
{
    ...
    /**
     * <p>
     * Encodes a byte array in Base64 format and writes to StringBuffer object. For regular blocks
     * writes only full lines and leaves the rest unused. Caller should check how many bytes were
     * actually enocded and return the rest at the beginning of the next block
     * </p>
     * 
     * @param raw
     *            <code>byte[]</code> data to be encoded
     * @param rawLen
     *            number of bytes used in raw
     * @param sb
     *            output buffer
     * @param bLastBlock
     *            true if this is the last block to write
            ...
            throws IOException
    {
        ...
        else
        {
            ...
            if (len3 < rawLen)
            {
                char[] encdata = encodeBlock(raw, len3);
                nTotal += encdata.length;
                sb.append(encdata);
            }
        }
        if (m_logger.isDebugEnabled())
            m_logger.debug("Input: " + rawLen + " used: " + nUsedBytes + " last: " + bLastBlock
                    + " wrote: " + nTotal);
        return nUsedBytes;
    }
    ...
    /**
     * <p>
     * Encodes a byte array in Base64 format and writes to output stream. For regular blocks writes
     * only full lines and leaves the rest unused. Caller should check how many bytes wre actually
     * enocded and return the rest at the beginning of the next block
     * </p>
     * 
     * @param raw
     *            <code>byte[]</code> data to be encoded
    ...
    protected static char[] encodeBlock(byte[] raw, int offset)
    {
        ...
        // System.out.println("Slack: " + slack + " offset" + offset);
     ...
     * Returns the number of bytes from input data used. Caller must pass in the unused bytes on the
     * next call.
     * </p>
     * 
     * @param base64
     *            <code>String</code> encoded string. Whitespace will be ignored.
     * @param out
     *            output stream to write decoded data
     * @param bLastBlock
    ...
    public static int decodeBlock(String base64, OutputStream out, boolean bLastBlock)
    {
        ...
        do
        {
            ...
            if (sbBlock.length() == 4)
            {
                ...
                int block = (getValue(sbBlock.charAt(0)) << 18)
                        + (getValue(sbBlock.charAt(1)) << 12) + (getValue(sbBlock.charAt(2)) << 6)
        ...
        if (m_logger.isDebugEnabled())
            m_logger.debug("Decoding: " + base64.length() + " last: " + bLastBlock + " used: "
                    + nUsed + " decoded: " + nDec);
        return nUsed;
    }
     ...
     * 
     * @param base64
     *            <code>String</code> encoded string (single line only !!)
