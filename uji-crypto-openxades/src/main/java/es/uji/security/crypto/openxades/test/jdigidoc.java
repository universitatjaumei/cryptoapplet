...
import es.uji.security.crypto.openxades.digidoc.xmlenc.*;
...
/**
 * jdigidoc is a small command-line programm providing an interface to most of the librarys
 * functionality and also documenting the library and serving as sample code for other developers.
 * 
...
public class jdigidoc
{
    ...
    /**
     * Checks for commands related to creating signed documents
     * 
     * @param args
    ...
    private boolean runNewSignedDocCmds(String[] args)
    {
        ...
        if (bFound)
        {
            ...
            {
                bOk = false;
                System.err.println("Error creating digidoc format: " + format + " version: "
                        + version + " - " + ex);
    ...
    /**
     * Checks for commands related to writing signed documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    private boolean runWriteSignedDocCmds(String[] args)
    {
        ...
        if (bFound && outFile != null)
        {
            try
            {
                ...
                {
                    System.err
                            .println("No signed document to sign. Use -ddoc-in or -ddoc-new commands!");
                    return false;
                }
    ...
    /**
     * Checks for commands related to adding data files to signed documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
            ...
            if (args[i].equals("-ddoc-add"))
            {
                ...
                if (inFile != null && inMime != null)
                {
                    try
                    {
                        if (m_sdoc == null)
                        {
                            System.out.println("Creating digidoc: " + SignedDoc.FORMAT_DIGIDOC_XML
                                    + ", " + SignedDoc.VERSION_1_3);
                            m_sdoc = new SignedDoc(SignedDoc.FORMAT_DIGIDOC_XML,
                                    SignedDoc.VERSION_1_3);
                        }
                        System.out.println("Adding data-file: " + inFile + ", " + inMime + ", "
                                + inContent);
    ...
    /**
     * Checks for commands related to displaying signed documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    /**
     * Checks for commands related to validating signed documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    /**
     * Checks for commands related to displaying encrypted documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    private boolean runListEncryptedDataCmds(String[] args)
    {
        ...
        if (bFound)
        {
            if (m_cdoc != null)
            {
                ...
                System.out.println();
                // display meta data
                System.out.println("\tFORMAT: " + m_cdoc.getPropFormatName() + " VER: "
                        + m_cdoc.getPropFormatVersion());
                System.out.println("\tLIBRARY: " + m_cdoc.getPropLibraryName() + " VER: "
                        + m_cdoc.getPropLibraryVersion());
                int nFiles = m_cdoc.getPropOrigFileCount();
                for (int i = 0; i < nFiles; i++)
                {
                    System.out.println("\tDF: " + m_cdoc.getPropOrigFileId(i) + " FILE: "
                            + m_cdoc.getPropOrigFileName(i) + " SIZE: "
                            + m_cdoc.getPropOrigFileSize(i) + " MIME: "
                            + m_cdoc.getPropOrigFileMime(i));
                }
                ...
                for (int i = 0; i < m_cdoc.getNumKeys(); i++)
                {
                    ...
                    if (ekey.getRecipientsCertificate() != null)
                        System.out.print("\n\t\tCERT: "
                                + ekey.getRecipientsCertificate().getSubjectDN().getName());
                    System.out.println();
                }
    ...
    /**
     * Checks for commands related to checking certificates
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    /**
     * Checks for commands related to reading signed documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    /**
     * Checks for commands related to reading encrypted documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    private boolean runEncryptEncryptedDataCmds(String[] args)
    {
        ...
        if (bFound)
        {
            ...
            else
            {
                bOk = false;
                System.err
                        .println("Missing input file or output file of the -cdoc-encrypt command");
            }
        }
        return bOk;
    }
    ...
    /**
     * Checks for commands related to decrypting encrypted documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    /**
     * Checks for commands related to decrypting large encrypted documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
    ...
    private boolean runDecryptEncryptedStreamCmds(String[] args)
    {
        ...
        if (bFound)
        {
            ...
            {
                System.out.println("Decrypting: " + inFile + " to: " + outFile + " recv: "
                        + recvName);
                try
                {
                    FileInputStream fis = new FileInputStream(inFile);
                    FileOutputStream fos = new FileOutputStream(outFile);
                    EncryptedStreamParser streamParser = ConfigManager.instance()
                            .getEncryptedStreamParser();
            ...
            else
            {
                bOk = false;
                System.err
                        .println("Missing input file, recipient name, pin or output file of the -cdoc-decrypt-stream command");
            }
        }
        return bOk;
    }
    ...
    /**
     * Checks for commands related to encrypting big files
     * 
     * @param args
     *            command line arguments
     * @return success flag
        ...
        if (bFound)
        {
            if (outFile != null && inFile != null)
            {
                ...
                try
                {
                    // TODO: check cdoc existencs
                    m_cdoc.addProperty(EncryptedData.ENCPROP_FILENAME, inFile);
                    m_cdoc.encryptStream(new FileInputStream(inFile),
                            new FileOutputStream(outFile), nCompressOption);
                    bOk = true;
                }
                catch (Exception ex)
                {
            ...
            else
            {
                bOk = false;
                System.err
                        .println("Missing input file or output file of the -cdoc-encrypt-stream command");
            }
        }
        return bOk;
    }
    ...
    /**
     * Checks for commands related to adding recipients (EncryptedKey -s) encrypted documents
     * 
     * @param args
     *            command line arguments
     * @return success flag
                ...
                if (certFile != null)
                {
                    try
                    {
                        if (m_cdoc == null)
                        {
                            ...
                            m_cdoc = new EncryptedData(null, null, null,
                                    EncryptedData.DENC_XMLNS_XMLENC,
                                    EncryptedData.DENC_ENC_METHOD_AES128);
                        }
                        System.out.println("Adding recipient: " + certFile + ", " + recipient
                                + ", " + keyName + ", " + carriedKeyName);
                        X509Certificate recvCert = SignedDoc.readCertificate(new File(certFile));
                        EncryptedKey ekey = new EncryptedKey(null, recipient,
                                EncryptedData.DENC_ENC_METHOD_RSA1_5, keyName, carriedKeyName,
                                recvCert);
    ...
    /**
     * run-loop for jdigidoc. Evaluates the command line arguments and executes the commands
     * 
     * @param args
     *            command line arguments
     */
    public void run(String[] args)
    {
        ...
        if (!bOk)
        {
            ...
            System.err
                    .println("\t-cdoc-recipient <certificate-file> [recipient] [KeyName] [CarriedKeyName]");
            ...
            System.err
                    .println("\t-cdoc-decrypt-stream <input-file> <recipient> <pin> <output-file>");
    ...
    public static void main(String[] args)
    {
        ...
        prog.run(args);
        de = new Date();
        System.out.println(SignedDoc.LIB_NAME + " end, time: "
                + ((de.getTime() - ds.getTime()) / 1000) + " [sek]");
    }
}
